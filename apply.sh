...
original_author="nezhdanchik"
original_name="storage"
original_urlname="storage"
original_description="Awesome storage created by nezhdanchik" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/storage "${name}"
...
then
    ...
    echo "then run 'storage' or 'python -m storage'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'storage' or 'python -m storage' or 'poetry run storage'"
fi
