...
- 🎯 Entry points to execute your program using `python -m <storage>` or `$ storage` with basic CLI argument parsing.
...
---
# storage Flask Application

Awesome storage created by nezhdanchik
...
```bash
git clone https://github.com/nezhdanchik/storage storage
cd storage
make install
```
...
```bash
storage create-db   # run once
storage populate-db  # run once (optional)
storage add-user -u admin -p 1234  # ads a user
storage run
```
